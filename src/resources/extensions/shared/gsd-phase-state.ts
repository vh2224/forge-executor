...
// biome-ignore lint/correctness/noUnusedFunctionParameters: signature kept stable for callers; audit emission removed (dead write-side, gsd auto-mode deleted).
function emitPhaseChange(_action: string, _previousPhase: string | null, _nextPhase: string | null): void {
	// no-op: audit emission removed with gsd/uok/audit (D2 — no gsd copy)
}
