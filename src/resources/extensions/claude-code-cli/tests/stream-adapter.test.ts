...
describe("stream-adapter — abort classification (F2)", () => {
	...
	// A subprocess hung mid-tool never yields the next SDK message, so the
	// top-of-loop abort gate is inert. The signal listener must settle the
	// stream anyway and tear the query down.
	test("settles within <1s when the SDK iterator is hung mid-tool (never yields)", async () => {
		const controller = new AbortController();
		let iteratorReturned = false;

		const stream = streamViaClaudeCode(
			{ id: "claude-sonnet-4-6" } as any,
			{ messages: [{ role: "user", content: "hang" } as Message] },
			{
				_skipWorkflowMcpPreflightForTest: true,
				signal: controller.signal,
				// Models the real SDK `Query` shape: the returned object is BOTH the
				// async-iterable and its own iterator, exposing `return`/`interrupt`
				// directly (Query extends AsyncGenerator). It yields NOTHING and never
				// completes — a subprocess stuck mid-tool. `return()` records the
				// best-effort teardown the abort listener performs.
				_sdkQueryForTest() {
					const query = {
		...
		// Let the pump reach the hung `next()`, then abort.
		await new Promise((r) => setTimeout(r, 20));
		const abortAt = Date.now();
		controller.abort();
		await drained;

		assert.ok(Date.now() - abortAt < 1000, "stream settled within 1s of the abort");
		const aborted = events.find((e) => e.type === "error" && e.reason === "aborted");
		assert.ok(aborted, "emitted an aborted error out-of-band, not gated on the next SDK message");
		assert.equal(iteratorReturned, true, "best-effort teardown called iterator.return()");
	});
});
