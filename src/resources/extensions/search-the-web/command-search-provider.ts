/**
 * /web-search-provider slash command and its deprecated /search-provider alias.
 *
 * Lets users switch between tavily, brave, and auto search backends.
 * Supports direct arg (`/web-search-provider tavily`) or interactive select UI.
...
type CommandOptions = Omit<Parameters<ExtensionAPI['registerCommand']>[1], 'description'>
...
export function registerSearchProviderCommand(pi: ExtensionAPI): void {
  ...
  pi.registerCommand('web-search-provider', {
    ...options,
    description: 'Switch WEB SEARCH backend (tavily/brave/ollama/auto) — not the LLM provider; use /model to change models',
  })
  pi.registerCommand('search-provider', {
    ...options,
    description: '(deprecated alias of /web-search-provider) — switch web search backend',
  })
}
