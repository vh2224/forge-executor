...
const CANONICAL_WORKTREES_DIRNAME = '.gsd-worktrees'

/**
 * The directories a GSD worktree may live under, canonical first then legacy
 * (`.gsd/worktrees`). Inline replacement for the removed gsd worktree-placement
 * helper — mirrors its `[canonicalWorktreesDir, legacyWorktreesDir]` shape.
 */
function worktreesDirs(projectRoot: string): string[] {
  return [
    join(projectRoot, CANONICAL_WORKTREES_DIRNAME),
    join(projectRoot, '.gsd', 'worktrees'),
  ]
}
