 ...
 * Inline list (harness core no longer depends on the gsd setup-catalog). Covers
 * the LLM providers the wizard offers plus 'anthropic-vertex' and 'ollama',
 * which are still treated as "authed = no onboarding needed" for back-compat.
 */
...
export async function runOnboarding(
  ...
  if (opts.showIntro !== false && process.env.GSD_SUPPRESS_LOGO !== '1') {
    ...
    p.intro(pc.bold('Welcome to Forge — let\'s get you set up'))
  ...
  const llmResult = await runStep(p, 'LLM setup failed', () => runLlmStep(p, pc, authStorage), {
    ...
    errorInfo: 'You can configure your LLM provider later with /login inside Forge.',
  })
  ...
  if (llmConfigured) {
    ...
    summaryLines.push(`${pc.yellow('↷')} LLM provider: skipped — use /login inside Forge`)
  }

  if (searchConfigured) {
    summaryLines.push(`${pc.green('✓')} Web search: ${searchConfigured}`)
  } else {
    summaryLines.push(`${pc.dim('↷')} Web search: not configured — use /search-provider inside Forge`)
  }

  if (remoteConfigured) {
    summaryLines.push(`${pc.green('✓')} Remote questions: ${remoteConfigured}`)
  } else {
    summaryLines.push(`${pc.dim('↷')} Remote questions: not configured — use /gsd remote inside Forge`)
  }
  ...
  p.outro(pc.dim('Launching Forge...'))
}
...
export async function runLlmStep(p: ClackModule, pc: PicoModule, authStorage: AuthStorage): Promise<boolean> {
  ...
  authOptions.push(
    ...
    { value: 'skip', label: 'Skip for now', hint: 'use /login inside Forge later' },
  )
...
async function runApiKeyFlow(
  ...
  if (providerId === 'openrouter') {
    p.log.info(`Use ${pc.cyan('/model')} inside Forge to pick an OpenRouter model.`)
...
export async function runWebSearchStep(
  ...
  options.push(
    ...
    { value: 'skip', label: 'Skip for now', hint: 'use /search-provider inside Forge later' },
  )
...
export async function runRemoteQuestionsStep(
  ...
  options.push(
    ...
    { value: 'skip', label: 'Skip for now', hint: 'use /gsd remote inside Forge later' },
  )

  const choice = await p.select({
    message: 'Set up remote questions? (get notified when Forge needs input)',
    options,
  })
