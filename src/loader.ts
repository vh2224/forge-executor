...
if (!existsSync(appRoot) && process.env.GSD_SUPPRESS_LOGO !== '1') {
  ...
  process.stderr.write(
    ...
    '\n' +
    `  ${bold}${GSD_PI_BRAND}${reset} ${dim}· v${gsdVersion}${reset}\n` +
...
// FORGE_CODING_AGENT_DIR — piConfig.name is now 'forge', so vendored pi's config.ts derives
// APP_NAME='forge' and ENV_AGENT_DIR='FORGE_CODING_AGENT_DIR' (not GSD_CODING_AGENT_DIR).
// Set both for back-compat (Decision 4b).
process.env.FORGE_CODING_AGENT_DIR = agentDir
...
// resourcesDir — bundled resources root. Prefers dist/resources/ (stable, set at
// build time) over src/resources/ (live working tree) — see resource-loader.ts
// for rationale.
