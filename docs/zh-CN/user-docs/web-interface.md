...
| `--no-auth` | 关闭 | 禁用内置 bearer token 认证 |

`--no-auth` 会让 Web 界面本身不受保护；仅应在反向代理、VPN 或私有网络边界等可信外部访问控制之后使用。
...
- **切换项目根目录**：无需重启服务器即可在 Web UI 中切换项目目录
...
| `GSD_WEB_NO_AUTH` | 设为 `1` 可禁用内置 Web bearer token 认证 |

## Node v24 兼容性

Node v24 对类型剥离（type stripping）做了破坏性改动，曾导致 Web 启动时报 `ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING`。该问题已在近期版本修复（#1864）。如果你仍然遇到这个错误，请升级 GSD。

## 认证令牌持久化

Web UI 会把认证令牌持久化到 `sessionStorage`，因此页面刷新后不会丢失登录态（#1877）。在此之前，每次刷新都需要重新认证。
