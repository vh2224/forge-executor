...
| **指标** | 项目根目录的 `.gsd/metrics.json` 仍是持久账本；worktree 诊断信息可能会镜像回那里 |
| **崩溃恢复** | 协调状态锚定在项目根目录；每个 worker 的锁和诊断只是实现细节，不是事实来源 |

### 协调方式

Workers 和 coordinator 通过 `.gsd/gsd.db` 中的数据库协调表通信：

- **`workers`**：记录活跃 auto-mode worker、heartbeat TTL、关停/崩溃状态
- **`milestone_leases`**：通过 fencing token 保证同一 milestone 同时只有一个 worker 拥有
- **`unit_dispatches`**：记录每个 unit 的 claim、running、completed、failed、stuck、canceled、retry 状态
- **`command_queue`**：coordinator 写入定向或广播控制命令，worker 原子 claim
...
`.gsd/parallel/`、`.gsd/worktrees/` 和 `.gsd-worktrees/` 都会被 gitignore，因为它们只是运行时协调文件，永远不会提交。
