...
**Update (2026-06-08):** GSD PR body generation now routes through the `buildPullRequestEvidence` wrapper in `pull-request-process.ts`, which forces `aiAssisted: false`, so generated PR bodies no longer emit the AI Assistance Disclosure section (per the repository convention not to credit AI tooling as an author in PRs/issues). All three consumers — `/gsd ship`, GitHub sync slice PRs, and swarm lane PRs — call the wrapper, and the TL;DR, linked-issue, tests-run, change-type, and rollback/compatibility sections are unchanged. The Phase 7 exit-gate and 2026-05-04 closeout note above describe the original design and remain accurate as of their dates; the AI-disclosure requirement no longer applies to generated bodies.
