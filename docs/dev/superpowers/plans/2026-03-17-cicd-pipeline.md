...
jobs:
  ...
  dev-publish:
    ...
    container:
      image: ghcr.io/open-gsd/gsd-ci-builder:latest  # Pin to date tag after first build
    environment: dev
    outputs:
      ...
      - name: Build and push runtime Docker image
        run: |
          ...
          docker build --target runtime \
            ...
            -t ghcr.io/open-gsd/gsd-pi:next \
            -t ghcr.io/open-gsd/gsd-pi:${{ needs.dev-publish.outputs.dev-version }} \
            .
          docker push ghcr.io/open-gsd/gsd-pi:next
          docker push ghcr.io/open-gsd/gsd-pi:${{ needs.dev-publish.outputs.dev-version }}
  ...
  prod-release:
    ...
    steps:
      ...
      - name: Tag and push Docker images
        run: |
          echo "${{ secrets.GITHUB_TOKEN }}" | docker login ghcr.io -u ${{ github.actor }} --password-stdin
          docker pull ghcr.io/open-gsd/gsd-pi:${{ needs.dev-publish.outputs.dev-version }}
          docker tag ghcr.io/open-gsd/gsd-pi:${{ needs.dev-publish.outputs.dev-version }} ghcr.io/open-gsd/gsd-pi:latest
      ...
      - name: Build and push CI builder image
        run: |
          ...
          docker build --target builder \
            ...
            -t ghcr.io/open-gsd/gsd-ci-builder:${{ steps.tag.outputs.date }} \
            .
          docker push ghcr.io/open-gsd/gsd-ci-builder:latest
          docker push ghcr.io/open-gsd/gsd-ci-builder:${{ steps.tag.outputs.date }}

      - name: Verify builder image
        run: |
          docker run --rm ghcr.io/open-gsd/gsd-ci-builder:latest node --version
          docker run --rm ghcr.io/open-gsd/gsd-ci-builder:latest rustc --version
```
...
4. **Enable GHCR:**
   - Ensure GitHub Container Registry is enabled for the `open-gsd` org
