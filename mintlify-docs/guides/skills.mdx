...
GSD ships with managed skills installed to `~/.gsd/agent/skills/`. User-global
skills belong in `~/.agents/skills/`, and project skills belong in
`.agents/skills/`.
...
`@opengsd/gsd-browser` package and refreshed on startup, including referenced
support files in `docs/`, `scripts/`, and `gsd-browser-skill/`.
...
1. **Bare name** — e.g., `frontend-design` → scans managed, global, and project skill directories
2. **Absolute path** — e.g., `/Users/you/.agents/skills/my-skill/SKILL.md`
3. **Directory path** — looks for `SKILL.md` inside

Managed GSD skills take precedence when resolving preference refs by bare name.
...
`SKILL.md` frontmatter must be valid YAML. Quote description values containing
`:` or use a folded block (`description: >`) for longer trigger descriptions.
