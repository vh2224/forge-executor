...
| `SCREENSHOT_MAX_WIDTH` | `1568` | Maximum width, in pixels, for inline image payloads returned by `browser_screenshot` and `mac_screenshot`; set to `0` to disable the width cap |
| `SCREENSHOT_MAX_HEIGHT` | `8000` | Maximum height, in pixels, for inline image payloads returned by `browser_screenshot` and `mac_screenshot`; set to `0` to disable the height cap |

Oversized `browser_screenshot` and `mac_screenshot` payloads are downscaled with aspect ratio preserved, so macOS screenshots follow the same sizing caps as browser screenshots.
