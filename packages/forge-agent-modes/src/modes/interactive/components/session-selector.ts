...
import { deriveSessionTitle, enrichSessionsWithWorkerSlice, type ThreadSessionInfo } from "../../../session-thread/index.js";
...
class SessionSelectorHeader implements Component {
	...
	private showWorkerSlices: boolean;
	...
	constructor(
		...
		this.showWorkerSlices = showWorkerSlices;
	...
	setShowWorkerSlices(showWorkerSlices: boolean): void {
		this.showWorkerSlices = showWorkerSlices;
	}
	...
	render(width: number): string[] {
		...
		const sliceLabel = this.showWorkerSlices ? "on" : "off";
		const sliceText = theme.fg("muted", "Fatias: ") + theme.fg("accent", sliceLabel);
		...
		const rightText = truncateToWidth(`${scopeText}  ${nameText}  ${sliceText}  ${sortText}`, width, "");
		...
		if (this.confirmingDeletePath !== null) {
			...
			const hint2Parts = [
				...
				appKeyHint(this.keybindings, "toggleWorkerSlices", "fatias"),
...
export class SessionList implements Component, Focusable {
	...
	private allSessions: ThreadSessionInfo[] = [];
	...
	public onToggleWorkerSlices?: () => void;
	...
	setSessions(sessions: ThreadSessionInfo[], showCwd: boolean): void {
		this.allSessions = sessions;
		this.showCwd = showCwd;
		this.filterSessions(this.searchInput.getValue());
	}
	...
	private filterSessions(query: string): void {
		...
		const sliceFiltered = this.showWorkerSlices
			? this.allSessions
			: this.allSessions.filter((session) => !session.workerSlice);
		const nameFiltered =
			this.nameFilter === "all" ? sliceFiltered : sliceFiltered.filter((session) => hasSessionName(session));
	...
	render(width: number): string[] {
		...
		if (this.filteredSessions.length === 0) {
			...
			if (!this.showWorkerSlices && this.allSessions.length > 0 && this.allSessions.every((s) => s.workerSlice)) {
				const toggleKey = appKey(this.keybindings, "toggleWorkerSlices");
				emptyMessage = `  Só havia fatias de worker ocultas. Pressione ${toggleKey} para mostrá-las.`;
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			const displayText = session.name ?? deriveSessionTitle(session);
	...
	handleInput(keyData: string): void {
		...
		if (this.keybindings.matches(keyData, "toggleWorkerSlices")) {
			this.onToggleWorkerSlices?.();
			return;
		}
...
export class SessionSelectorComponent extends Container implements Focusable {
	...
	private sortMode: SortMode = "threaded";
	private nameFilter: NameFilter = "all";
	private showWorkerSlices = false;
	private currentSessions: ThreadSessionInfo[] | null = null;
	private allSessions: ThreadSessionInfo[] | null = null;
	...
	constructor(
		...
		this.sessionList.onToggleWorkerSlices = () => this.toggleWorkerSlices();
	...
	private async loadScope(scope: SessionScope, reason: "initial" | "refresh" | "toggle"): Promise<void> {
		...
		try {
			const rawSessions = await (scope === "current"
				? this.currentSessionsLoader(onProgress)
				: this.allSessionsLoader(onProgress));
			const sessions = await enrichSessionsWithWorkerSlice(rawSessions);
	...
	private toggleWorkerSlices(): void {
		this.showWorkerSlices = !this.showWorkerSlices;
		this.header.setShowWorkerSlices(this.showWorkerSlices);
		this.sessionList.setShowWorkerSlices(this.showWorkerSlices);
		this.requestRender();
	}
