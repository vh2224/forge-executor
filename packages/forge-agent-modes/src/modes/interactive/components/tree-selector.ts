...
import type { SessionTreeNode } from "@gsd/pi-coding-agent/core/session-manager.js";
		...
		switch (entry.type) {
			case "message": {
				...
				if (role === "user") {
					...
					if (toolCall) {
						result = theme.fg("muted", formatToolCallSummary(toolCall.name, toolCall.arguments));
					} else {
