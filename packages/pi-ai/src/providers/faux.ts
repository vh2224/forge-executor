...
function assistantContentToText(
	content: Array<TextContent | ThinkingContent | ToolCall | ServerToolUse | WebSearchResult>,
): string {
	return content
		.map((block) => {
			if (block.type === "text") {
			...
			return `webSearchResult:${block.toolUseId}`;
		})
		.join("\n");
}
...
async function streamWithDeltas(
	...
	for (let index = 0; index < message.content.length; index++) {
		...
		if (block.type === "serverToolUse" || block.type === "webSearchResult") {
