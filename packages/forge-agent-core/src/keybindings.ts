...
export interface AppKeybindings {
	...
	"app.session.toggleWorkerSlices": true;
...
export const KEYBINDINGS = {
	...
	"app.session.toggleWorkerSlices": {
		// ctrl+w conflicts with the search Input's deleteWordBackward (pi-tui
		// editor-keybindings.ts) — verified free of both editor and app-level
		// bindings before picking it (S03/T02).
		defaultKeys: "ctrl+q",
		description: "Toggle worker slice visibility",
	},
	"app.editor.external": {
		defaultKeys: "ctrl+g",
...
const KEYBINDING_NAME_MIGRATIONS = {
	...
	toggleWorkerSlices: "app.session.toggleWorkerSlices",
...
/** Legacy GSD app action names (mapped to app.* keybindings). */
...
const LEGACY_APP_ACTIONS = new Set<string>([
	"interrupt",
	"clear",
	"exit",
	"suspend",
...
function resolveKeybinding(action: AppKeybinding | AppAction): keyof import("@gsd/pi-tui").Keybindings {
	if (LEGACY_APP_ACTIONS.has(action)) {
		return KEYBINDING_NAME_MIGRATIONS[action as keyof typeof KEYBINDING_NAME_MIGRATIONS] as keyof import("@gsd/pi-tui").Keybindings;
	}
	return action as keyof import("@gsd/pi-tui").Keybindings;
}
...
export class KeybindingsManager extends TuiKeybindingsManager {
	...
	matches(data: string, keybinding: keyof import("@gsd/pi-tui").Keybindings | AppAction): boolean {
		return super.matches(data, resolveKeybinding(keybinding as AppKeybinding | AppAction));
	}

	getKeys(keybinding: keyof import("@gsd/pi-tui").Keybindings | AppAction): KeyId[] {
		return super.getKeys(resolveKeybinding(keybinding as AppKeybinding | AppAction));
	}
}
